"""
VMamba + Faster R-CNN 检测配置（带注意力机制）
使用 ZoomInSelfAttention 增强 FPN 输出特征
"""
# custom_imports = dict(imports=['classification.models'], allow_failed_imports=False)
_base_ = [
...
# 模型配置（使用 VMamba-Tiny 作为 backbone + 增强的 FPN with 注意力）
model = dict(
    ...
    neck=dict(
        ...
        out_channels=256,
        num_outs=5,  # FPN 输出5个尺度：P2, P3, P4, P5, P6
        # 注意力配置
        attention_cfg=dict(
            type='ZoomInSelfAttention',
            num_heads=4,        # 多头注意力头数
            reduction=16,       # SE 通道注意力压缩比
            # kv_downsample=2,   # K/V 降采样倍数（2表示降采样到1/2，大幅降低显存占用）
            kv_downsample=8,  # K/V 改为4再次测试
        ),
        # 只在 P3, P4, P5 上应用注意力（跳过 P2 和 P6，降低显存占用）
        attention_indices=[0, 1, 2, 3],  # 只在 P3, P4, P5 上应用
    ),
...
# 工作目录
work_dir = './work_dirs/windfarm_detection_with_attention'
